module UnifiedPaymentService
  module UnifiedPaymentUtility
    ...
    def create_order_at_unified(amount)
      3.times do |attempt|
        begin
          @response = UnifiedPaymentClient.create_order(amount, { :approve_url => "#{root_url}pay_by_cards/approved", :cancel_url => "#{root_url}pay_by_cards/canceled", :decline_url => "#{root_url}pay_by_cards/declined" }) 
          break
        rescue
    ...
    def get_unified_order_status
      response = UnifiedPaymentClient.get_order_status(gateway_order_id, gateway_session_id)
